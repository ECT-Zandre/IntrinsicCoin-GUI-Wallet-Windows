...
const char DEFAULT_WALLET_FILE_NAME[] = "intrinsicoinwallet.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "localhost:35354";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("intrinsicoinwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("intrinsicoinwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("intrinsicoinwallet.desktop");
  ...
  res = autorunSettings.contains("IntrinsicoinWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("IntrinsicoinWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.intrinsicoin.intrinsicoinwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Intrinsicoin Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("IntrinsicoinWallet", appPath);
    } else {
      autorunSettings.remove("IntrinsicoinWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\intrinsicoin", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:intrinsicoin");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\intrinsicoin\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\intrinsicoin\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("intrinsicoinwallet.cfg"));

...
const char BYTECOIN_URI_SCHEME_NAME[] = "intrinsiccoin";
...
WalletApplication::WalletApplication(int& _argc, char** _argv) : QApplication(_argc, _argv), m_lockFile(nullptr),
  ...
  setApplicationName("intrinsiccoinwallet");
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("intrinsiccoinwallet.lock"));
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] Intrinsic Coin wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "Intrinsic Coin wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have intrinsiccoind with non-default RPC port, you should terminate it and relaunch IntrinsicCoinWallet\n"
      "or\n"
      "Set the Local deamon required port in IntrinsicCoinWallet Menu/Preferences/Connection settings."));
      dlg.exec();
      return false;
    }
